...
建议使用生成的 vRPC client 访问这条路径。App 与 Link 端点是运行时内部协议
...
spec 而异。Vine 的兼容保证只覆盖由 skelc 管理的生成 package，不包括加入其中
的手写文件；package 所有权规则见 [Skel Go 生成](https://skel.yorun.ai/docs/generation/go)。
