...
完成后，你会得到一个可启动、可优雅停止的 Vine 应用，以及一个本地 SQLite 数据库文件。
...
- **Link** 注册 `HelloApp`，提供配置、服务发现和请求转发能力。
...
此时业务应用与一个 Link 在同一进程，Link 会连接独立 Hub，并向其注册应用能力。`HubEndpoint` 与 `IngressListen` 也可以分别由 `VINE_HUB_ENDPOINT` 和 `VINE_INGRESS_LISTEN` 提供。

如需把 Link 与业务应用拆为独立进程，则使用 `app.New` 启动业务应用，并通过 `--link-endpoint` 或 `VINE_LINK_ENDPOINT` 指定已有 Link 的 API endpoint。

## 下一步

- 完成 [第一个 Skel 契约](/docs/first-skel-contract)，生成类型安全的业务代码。
- 阅读 [App](/docs/app)，了解应用、组件和模块的完整生命周期。
- 阅读 [Hub](/docs/hub)、[Link](/docs/link) 和 [Portal](/docs/portal)，理解多进程部署的职责边界。
- 阅读 [使用 Rpc](/docs/guide/rpc)，开始声明并调用服务。
