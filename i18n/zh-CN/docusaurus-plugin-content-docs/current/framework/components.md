...
业务能力通常写成 Module。Module 适合承载领域服务、后台工作和需要跟随应用启停的资源：
...
数据库、Redis 等基础设施以组件形式接入。业务应用声明具体 Component 类型，并在 Component 中提供连接参数、DAO、Locker 或 Cache：
...
Component 对外暴露的对象会进入依赖注入容器。Module、RPC Handler、Web Handler、Event Listener 和 Task Runner 都能直接注入使用。
...
flowchart LR
  ...
  AfterStart --> BeforeStop["BeforeAppStop"] --> Stop["注销、排空并停止"] --> AfterStop["AfterAppStop"]
```
...
- `BeforeAppStop`：停止生产新工作，为注销和排空阶段做好准备。
- `AfterAppStop`：释放连接和其他资源。

启动时，Vine 先按声明顺序执行基础设施组件，再执行 Module。停止时先以相反顺序停止 Module，再停止 Component。这样业务模块在启动时就能使用已经就绪的数据库和 Redis，在释放这些连接前也有机会完成清理。

同类 Module 或 Component 不能重复声明。需要共享依赖时，在应用的 `BindCommon` 或对象自己的 `Bind` 中绑定；一次请求内的上下文依赖交给 execution scope。
