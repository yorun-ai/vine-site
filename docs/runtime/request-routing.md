...
Method specs generated by skelc v0.12.0 or later structurally clone in-process
Rpc arguments and results when running on Vine v0.13.1 or later. Specs generated
by skelc v0.11.x omit those hooks, so Vine uses a serialization-based
compatibility fallback. In either case, caller and handler mutations cannot
...
| Preserved in standalone | Requires a real transport or process boundary |
| --- | --- |
...
| Request validation and value isolation | JSON/CBOR behavior and codec failures |
