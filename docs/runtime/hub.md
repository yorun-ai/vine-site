...
Deployment-managed component credentials and encrypted transport remain TODOs.
...
key, and subscription ACLs. The `vine.hub` user has a random process-local password, but the
Link and Portal passwords are temporarily empty. Their usernames select an ACL
role without proving the caller's identity, so any client that can reach the
endpoint can still impersonate either role; the Portal role can read Portal TLS
private keys. Deploy the Hub API, Hub Redis, Link API, and Link ingress only on
loopback or trusted private networks, restrict access with a firewall, and never
expose these internal ports to an untrusted network.
