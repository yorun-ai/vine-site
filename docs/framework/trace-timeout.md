...
Portal will create a span and use it as the entry anchor for the server-side call tree. This synthetic span doesn't correspond to real client-side logs, but it keeps server-side auth/check/target calls under the same trace tree.
...
External clients shouldn't rely on `vweb-actor` or `vweb-initiator`. webgw writes those headers for backend applications, and client-supplied values aren't trusted.
...
Clients can log this value and use it for later investigation. It only contains the trace id. It doesn't contain span data and isn't a full trace context for the next request.
...
When a business handler calls Rpc, it normally doesn't need to set timeout manually. As long as the call uses the current injected context, the Rpc client reads the context deadline and writes the remaining time to `vrpc-options`.
...
`meta.Trace` isn't a full OTel span. It only creates and propagates:
